...
import itgloria.reactive.uploadweb.config.S3ClientConfigurarionProperties;
import itgloria.reactive.uploadweb.exception.UploadFailedException;
...
public class UploadResource {
      ...
      .map(
        response -> {
          ...
          return ResponseEntity
            ...
            .body(
              new UploadResult(HttpStatus.CREATED, new String[] { fileKey })
            );
        }
      );
  }
  ...
  public Mono<ResponseEntity<UploadResult>> multipartUploadHandler(
    ...
    log.info("*** multipartUploadHandler no temporary storage ***");
    return parts
   ...
   * @return
   */
  protected Mono<String> saveFile(
    HttpHeaders headers,
    String bucket,
    FilePart part
  ) {
    ...
    log.info(
      "[I137] saveFile: filekey={}, filename={}",
      filekey,
      part.filename()
    );
  ...
  private Mono<CompletedPart> uploadPart(
    ...
    log.info(
      "[I218] uploadPart: partNumber={}, contentLength={}",
      partNumber,
      buffer.capacity()
    );
    ...
    return Mono
      ...
      .map(
        uploadPartResult -> {
          ...
          log.info(
            "[I230] uploadPart complete: part={}, etag={}",
            partNumber,
